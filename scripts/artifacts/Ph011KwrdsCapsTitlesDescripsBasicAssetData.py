__artifacts_v2__ = {
    'Ph011_1KwrdsCapsTitlesDescripsLikesBasicAsstDataPhDaPsql': {
        ...
        'category': 'Photos.sqlite-Assets-withDescriptions-PhotoData-Psql',
        'notes': '',
    ...
    'Ph011_3KwrdsCapsTitlesDescripsLikesBasicAsstDataGenPlayPsql': {
        ...
        'category': 'Photos.sqlite-Assets-withDescriptions-GenPlaygrndPL-Psql',
        'notes': '',
...
@artifact_processor
def Ph011_1KwrdsCapsTitlesDescripsLikesBasicAsstDataPhDaPsql(files_found, report_folder, seeker, wrap_text, timezone_offset):
    ...
    if (version.parse(iosversion) >= version.parse("14")) & (version.parse(iosversion) < version.parse("15")):
        ...
        FROM ZASSET zAsset
            ...
            LEFT JOIN ZCLOUDMASTER zCldMast ON zAsset.ZMASTER = zCldMast.Z_PK
            LEFT JOIN Z_1KEYWORDS z1KeyWrds ON zAddAssetAttr.Z_PK = z1KeyWrds.Z_1ASSETATTRIBUTES
            LEFT JOIN ZKEYWORD zKeywrd ON z1KeyWrds.Z_52KEYWORDS = zKeywrd.Z_PK
            LEFT JOIN ZASSETDESCRIPTION zAssetDes ON zAssetDes.Z_PK = zAddAssetAttr.ZASSETDESCRIPTION
            LEFT JOIN ZCLOUDFEEDENTRY zCldFeedEnt ON zAsset.ZCLOUDFEEDASSETSENTRY = zCldFeedEnt.Z_PK
            LEFT JOIN ZCLOUDSHAREDCOMMENT zCldSharedComment ON zAsset.Z_PK = zCldSharedComment.ZCOMMENTEDASSET
            LEFT JOIN ZCLOUDSHAREDCOMMENT zCldSharedCommentLiked ON zAsset.Z_PK = zCldSharedCommentLiked.ZLIKEDASSET
...
@artifact_processor
def Ph011_3KwrdsCapsTitlesDescripsLikesBasicAsstDataGenPlayPsql(files_found, report_folder, seeker, wrap_text, timezone_offset):
    for source_path in files_found:
        source_path = str(source_path)
