__artifacts_v2__ = {
    "familyHubAccount": {
        ...
        "notes": "Read from Library/Preferences/com.samsung.familyhub.plist, including the "
                 ...
                 "predates the first launch by several years on the tested extraction and is not "
                 "reported. Phone is the msisdn the session dictionary holds, as stored, and was "
                 "empty on the tested extraction. Family Status and Food Recognition Supported "
                 "are reported as stored. The app's Realm store (Documents/default.realm, file "
                 "format 9) was read and is deliberately not reported: its 1,851 rows of "
                 "class_FHFoodItemModel are the food catalogue the app downloads from the Whisk "
                 "service, which the cache shows arriving in 38 paginated responses holding "
                 "exactly 1,851 products, and every user-data class in that store held no rows "
                 "on the tested extraction, namely class_FHFoodListModel (fridge inventory), "
                 "class_FHShoppingListModel, class_FHToDoListModel, class_FHToDoItemModel, "
                 "class_FHMemoListModel, class_FHWhiteBoardListModel, class_FHCalendarModel, "
                 "class_FHCalendarEventModel, class_FHDeals, class_FHGlazeCameraInfoModel "
                 "(refrigerator camera images) and class_FHInsideFridgeScannedItemModel. "
                 "Documents/FamilyBoard/offlineDB held no rows either. So a row here records the "
                 "account and setup values the app stored, and on the tested extraction every "
                 "user-data class held no rows. The "
