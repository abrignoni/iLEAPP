__artifacts_v2__ = {
    "textnow_ios_messages": {
        ...
        "notes": "One row per row of ZTMOINTERACTION that carries no call identifier, "
                 ...
                 "JPEG. That cache folder is named for the image library rather than "
                 "for the app, so an entry is only used for a store in the same "
                 "container. Conversation is the name or address the conversation is "
                 "with, and it is blank on 30 rows because the store left them with no "
                 "conversation link; all 30 are on the iOS 13.3.1 image and are the "
                 "app's own inbox and marketing messages, which carry a numeric author "
                 "identifier and, on eleven of them, a local path to an image a "
                 "marketing library had cached rather than message text. Two further "
                 "rows carry such a path and do have a conversation link, so thirteen "
                 "rows in all hold one. Deleted State is reported as stored and is 0 on "
                 "60 rows and 2 on 4; the four rows marked 2 still hold their full "
                 "text. Read State, Send Status and Image Type are reported as stored "
                 "and are not translated. Image Type is blank on 47 rows. Thumbnail "
                 "Width and Height are filled only on the 4 picture rows. The store "
                 "also holds ZTMOSIPDATA, ZTMOBLOCKEDCONTACT, ZTMOCALLERIDENTRY, "
                 "ZTMOPURCHASETRANSACTION, ZTMOQOSTESTPROFILE and ZTMOQOSTESTSERVER, "
                 "all empty on both tested images, and Documents/ILDRates.sqlite, which "
                 "is a downloaded table of international dialling rates and is read by "
                 "nothing here.",
