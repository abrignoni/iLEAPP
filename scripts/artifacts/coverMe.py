__artifacts_v2__ = {
    ...
    "coverme_ios_messages": {
        ...
        "notes": "One row per row of the sms table in Documents/miliao.db. The app names every "
                 ...
                 "number, 7 rows and 5 rows on the one tested image, and **which end of the "
                 "message that column names was not established**, so no direction is reported "
                 "and the flag columns are left as stored. Timestamp is field5, Unix seconds, "
                 "reported in UTC, and all twelve rows fall within about seventy minutes of one "
                 "day inside the period the image covers. Three rows name an attachment path. "
                 "Two of them sit under Documents/uploadMedia and both resolved to a file in the "
                 "same app container, so the byte sizes reported are of the files themselves; "
                 "the third names a folder under Documents whose name is a hash, which this "
                 "artifact does not gather, so it is reported by path alone. **No picture is "
                 "shown**: the attachment files that did resolve begin with the same four bytes "
                 "as each other and carry no signature of any image or video format, so they are "
                 "reported by path, presence and size and are not decoded. A third file sits in "
                 "the same upload folder under a longer name that no row names. Five tables in "
                 "this database are read by nothing here and are named so the omission is "
                 "visible. field6 (as stored) held the value 1 on all twelve rows, too few "
                 ...
                 "not read as a conversation; and the keychain, key, password and successful "
                 "login tables. "
