...
def get_ph51possibleoptimizedassetsphdapsql(files_found, report_folder, seeker, wrap_text, timezone_offset):
		...
		if version.parse(iosversion) < version.parse("14"):
			logfunc(
				"Unsupported version for PhotoData/Photos.sqlite ZINTERNALRESOURCE table data from iOS " + iosversion)
		if (version.parse(iosversion) >= version.parse("14")) & (version.parse(iosversion) < version.parse("15")):
			...
			if usageentries > 0:
				...
				report.start_artifact_report(report_folder, 'Ph51-Possible_Optimized_Assets_IntResou-PhDaPsql',
											 description)
				report.add_script()
