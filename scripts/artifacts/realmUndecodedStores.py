__artifacts_v2__ = {
    "realmUndecodedStores": {
        ...
        "notes": "This artifact reports nothing about the contents of any database. It exists so "
                 ...
                 "indistinguishable from an empty one. The parser reads both the Cluster table "
                 "layout and the older pre-Cluster layout, so a store in either layout is "
                 "decoded and is not reported here. On the tested extractions the only stores it "
                 "returned no classes for were ones whose header could not be read at all. A row "
                 "is emitted only when the file holds more than 1024 non-zero bytes, which "
                 "excludes uninitialised stores: the two stores reported held 4,133 and 4,136 "
                 "non-zero bytes while an uninitialised one held 204. Header Read separates two "
                 "conditions. False means the file does not begin with the Realm mnemonic, so no "
                 "header could be read and File Format Version (as stored) is empty; whether the "
                 "file is encrypted, damaged or not a Realm store is not established here. True "
                 "with a file "
