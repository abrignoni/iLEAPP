__artifacts_v2__ = {
    "tiktok_replied": {
        ...
        "notes": (
            ...
            "path reconstructed from an iTunes backup names the container by its AppDomain "
...
# The IM store names are not unique to TikTok, so a matched file is only parsed
# when the app that owns its container is TikTok.
...
def tiktok_replied(context):
    ...
    if not aweme_dbs:
        logfunc("No TikTok-owned AwemeIM.db found. TikTok replied messages cannot be parsed.")
        return (), [], ""
