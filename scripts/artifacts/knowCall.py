...
def get_knowCall(files_found, report_folder, seeker):
	...
	if version.parse(iOSversion) < version.parse("12"):
		...
		cursor.execute('''
		select
		datetime(zobject.zcreationdate+978307200,'unixepoch') as "timed", 
			case zobject.zstartdayofweek 
			when "1" then "sunday"
			when "2" then "monday"
			when "3" then "tuesday"
	...
	if version.parse(iOSversion) >= version.parse("12"):
		...
		left join
			...
			on zobject.zsource = zsource.z_pk 
		where
			zstreamname is "/app/activity" and ("bundle id" = "com.apple.mobilesafari" or "bundle id" = "com.apple.safari")
			''')
	else:
		cursor = db.cursor()
		cursor.execute('''	
	...
	if version.parse(iOSversion) >= version.parse("12"):
		...
		""")
	elif version.parse(iOSversion) < version.parse("12"):
		cursor = db.cursor()
		cursor.execute(
		"""
		select
	...
	if version.parse(iOSversion) >= version.parse("11"):
		...
		if usageentries > 0:
			...
			logfunc('No data available in Battery Level ')
	...
	if version.parse(iOSversion) >= version.parse("12"):			
		...
		left join
			zsource 
			on zobject.zsource = zsource.z_p
				''')
