...
def get_voicemail(files_found, report_folder, seeker, wrap_text):
    ...
    if len(files_found) > 0:
        ...
        if usageentries > 0:
            data_list = []
            for row in all_rows:
                audio_file = join(report_folder, f'{row[4]}.amr')
                audio_tag = f''' 
                        ...
                        '''
                data_list.append(
                    (row[0], row[1], row[2], row[3], audio_tag))

        report = ArtifactHtmlReport('Voicemail')
        report.start_artifact_report(report_folder, 'Voicemail')
        report.add_script()
        data_headers = ('Date and time', 'Sender', 'Receiver', 'Duration', 'Audio File')
        ...
        if usageentries > 0:
            data_list = []
            for row in all_rows:
                audio_file = join(report_folder, f'{row[5]}.amr')
                audio_tag = f''' 
                        <audio controls src="{audio_file}" type="audio/amr" preload="none">
                            <p>Your browser does not support HTML5 audio elements.</p>
                        </audio> 
        ...
        report.add_script()
        data_headers = ('Date and time', 'Sender', 'Receiver', 'Duration', 'Trashed date', 'Audio File')
        report.write_artifact_data_table(data_headers, data_list, dirname(file_found), html_no_escape=['Audio File'])
        report.end_artifact_report()
